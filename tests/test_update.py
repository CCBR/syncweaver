...
def test_update_refreshes_when_previous_git_sha_is_unreachable(tmp_path, monkeypatch):
    """Verify `update` refreshes content when previous_git_sha is unreachable.

    Simulates a lockfile pointing at a commit that no longer exists in the
    source repository (e.g. its tracked branch was rebased, force-pushed, or
    deleted upstream). The diff cannot be computed, so `update` should skip
    it, treat the source as changed, and still succeed and refresh the
    lockfile instead of crashing with a git error.
    ...
    lock_data["sources"]["code/package1"]["git_sha"] = unreachable_sha
    ...
    source_entry = new_lock["sources"]["code/package1"]
    assert source_entry["git_sha"] != unreachable_sha
